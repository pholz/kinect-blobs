...
namespace cinder{
	...
	void CvBlobTracker::setListener( CvBlobListener* _listener ) {
		listener = _listener;
	}


	int CvBlobTracker::findOrder( int id ) {
	...
	CvTrackedBlob&  CvBlobTracker::getById( int id ) {
		// This is a bit inefficient but ok when
		// assuming low numbers of blobs
		// a better way would be to use a hash table
		for( int i=0; i<blobs.size(); i++ ) {
	...
	void CvBlobTracker::draw( float x, float y ) {
		gl::enableAlphaBlending();
		gl::color(ColorA( 1.0f,0.0f,.8f,.6f ));
		glPushMatrix();
		...
		for( int i=0; i<blobs.size(); i++ ) {
			...
			gl::drawString( docstring.str(), blobs[i].centroid);
	 ...
	 */
	void CvBlobTracker::trackBlobs( const vector<CvBlob>& _blobs ) {
		unsigned int i, j, k;
		...
		for( i=0; i<_blobs.size(); i++ ) {
			blobs.push_back( CvTrackedBlob(_blobs[i]) );
		}
		
		vector<CvTrackedBlob> *prev = &history[history.size()-1];
		...
		if( best_error_ndx != -1 ) {
			for( i=0; i<cursize; i++ ) {
				...
				if( blobs[i].id != -1 ) {
					CvTrackedBlob *oldblob = &(*prev)[matrix[best_error_ndx][i]];
					...
					blobs[i].deltaLoc = Vec2f( 0.0f, 0.0f );
					blobs[i].deltaArea = 0;
					blobs[i].predictedPos = blobs[i].centroid;
					blobs[i].deltaLocTotal = Vec2f( 0.0f, 0.0f );
				}
			}
		}
		...
		// assign IDs for any blobs that are new this frame (ones that didn't get
	...
	//
	//
	void CvBlobTracker::doBlobOn( const CvTrackedBlob& b ) {
		if( listener != NULL ) {
	...
	void CvBlobTracker::doBlobMoved( const CvTrackedBlob& b ) {
		if( listener != NULL ) {
	...
	void CvBlobTracker::doBlobOff( const CvTrackedBlob& b ) {
		if( listener != NULL ) {
	...
	//
	//
	inline void CvBlobTracker::permute( int start ) {
		if( start == ids.size() ) {
	...
	inline bool CvBlobTracker::checkValidNew( int start ) {
		int newidcount = 0;
		newidcount ++;
		for( int i=0; i<start; i++ ) {
			if(ids[i] == -1) {
	...
	inline bool CvBlobTracker::checkValid( int start ) {
		for(int i=0; i<start; i++) {
